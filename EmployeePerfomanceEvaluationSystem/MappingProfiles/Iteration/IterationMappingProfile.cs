    ...
    public class IterationMappingProfile : Profile
    {
        public IterationMappingProfile()
        {
            ...
            CreateMap<EmployeePerfomanceEvaluationSystem.Models.Iteration, IterationReposnseModel>()
                    ...
                    .ForMember(x => x.StartDate, opt => opt.MapFrom(src => src.StartDate.ToString("MM/dd/yyyy")))
                    .ForMember(x => x.EndDate, opt => opt.MapFrom(src => src.EndDate.ToString("MM/dd/yyyy")))
                    .ForMember(x => x.CreatedDate, opt => opt.MapFrom(src => src.CreatedDate.ToString("MM/dd/yyyy")))
                    .ForMember(x => x.ActivatedDate, opt => opt.MapFrom(src => (src.ActivatedDate == null) ? "N/A"
                                                                    : src.ActivatedDate.Value.ToString("MM/dd/yyyy")))
                    .ForMember(x => x.LockedDate, opt => opt.MapFrom(src => (src.LockedDate == null) ? "N/A"
                                                                    : src.LockedDate.Value.ToString("MM/dd/yyyy")));
        }
    }
}
